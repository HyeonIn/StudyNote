...
<%if(startPage>1){%>
	<button class="btn btn-light" onclick="location.href='board?action=list&page=<%=startPage-1%>'">이전</button>
<%} %>
<%for(int p = startPage; p <= endPage; p++) { %>
	<button class="btn btn-light" onclick="location.href='board?action=list&page=<%=p%>'"><%=p%></button>
<%} %>
<%if(endPage<totalPage){%>
	<button class="btn btn-light" onclick="location.href='board?action=list&page=<%=endPage+1%>'">다음</button>
<%} %>
