...
public class BoardDao {
	...
	public int deleteOne(int bno) {  //글 읽기
		...
		try {
			...
			System.out.println("deleteOne error");
			ex.printStackTrace();
		}finally {
			DBUtil.close(rs, pstmt, conn);
		}
	...
	public int updateOne(int bno, String title, String content) {  //글 읽기
		Connection conn = null;
		PreparedStatement pstmt = null;
		int result = 0;
		ResultSet rs = null;
		try {
			...
			String sql = "update board_tb set title = ?, content = ?\r\n" + 
					"where bno = ?;";
			pstmt = conn.prepareStatement(sql);
			...
			System.out.println("updateOne error");
			ex.printStackTrace();
		}finally {
			DBUtil.close(rs, pstmt, conn);
		}
